...
void main() {
  group('Pub Interaction', () {
    ...
    test('Install package to cache works', () async {
      final packagePath = await PubInteraction.installPackageToCache(
        'every_test',
        '1.0.0',
      );
      ...
      expect(
        File(path.join(packagePath, 'pubspec.yaml')).existsSync(),
        isTrue,
        reason:
    ...
    test(
      ...
      () async {
        ...
        try {
          ...
          final packages =
              PubInteraction.getAllPackageVersionsForPackageInCache(
                packageName,
              );
