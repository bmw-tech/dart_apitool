...
void main() {
  group('Private types exclusion', () {
    ...
    test('should not include private class in extracted API', () {
      final privateClassDeclarations = packageApi.interfaceDeclarations.where(
        (id) => id.name == '_PrivateClass',
      );
      expect(
        privateClassDeclarations,
        isEmpty,
        reason:
    ...
    test('should not include private enum in extracted API', () {
      final privateEnumDeclarations = packageApi.interfaceDeclarations.where(
        (id) => id.name == '_PrivateEnum',
      );
      expect(
        privateEnumDeclarations,
        isEmpty,
        reason:
    ...
    test('should not include private property in extracted API', () {
      final privatePropertyDeclarations = packageApi.fieldDeclarations.where(
        (fd) => fd.name == '_privateProperty',
      );
      expect(
    ...
    test('should not include any declarations from private_types.dart', () {
      ...
      final privateTypesDeclarations = allDeclarations.where(
        (d) => d.relativePath.endsWith('private_types.dart'),
      );

      expect(
        privateTypesDeclarations,
        isEmpty,
        reason:
    ...
    test('should verify that public types are still extracted', () {
      ...
      final classADeclarations = packageApi.interfaceDeclarations.where(
        (id) => id.name == 'ClassA',
      );
      expect(
        classADeclarations,
      ...
      final classCTypeAliases = packageApi.typeAliasDeclarations.where(
        (tad) => tad.name == 'ClassC',
      );
      expect(
        classCTypeAliases,
